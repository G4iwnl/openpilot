...
After a lane change starts, its path, lateral motion, and steering shape are led by the predicted path generated by the comma driving model. The controller follows that path within the vehicle's steering limits. g4Pilot therefore has **no normal lane-change-specific setting that directly commands a target steering angle, lateral speed, or completion time.**

g4Pilot mainly decides **when to send the lane-change request to the model**, not the shape of the maneuver. Its entry gates use the turn signal, configured delay, driver steering-torque confirmation, lane-marking type, the vehicle BSD signal, and available side-radar information.
...
| g4Pilot entry gates | Wait before entry and evaluate configured torque, lane-line, BSD, and side-object conditions | Guaranteeing a safe gap or designing the lane-change angle |
...
3. g4Pilot enters `preLaneChange` and evaluates the configured delay and entry gates.
