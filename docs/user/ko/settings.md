...
g4Pilot의 세부 설정은 **Carrot Web에서 모두 확인하고 변경하는 것**을 기준으로 합니다. 장치 자체의 기본 설정 화면은 Wi-Fi, 장치 정보, openpilot 기본 토글과 소프트웨어 업데이트에 사용하고, 아래에서 설명하는 `carrot_settings.json`의 파라미터는 Carrot Web의 **설정** 화면에서 관리합니다.
...
`g4` 탭은 이 포크에서만 제공하는 3개 항목입니다. 모두 기본값이 `0`(사용 안 함)이므로 켜기 전까지는 기존 동작과 같습니다.

| 중분류 | 파라미터 | 용도 |
|---|---|---|
| 크루즈 | `SoftHoldOnly` | 오토크루즈 기능 중 소프트홀드만 남기기 |
| 레이더 | `VisionOnly` | 레이더를 무시하고 비전으로만 주행 |
| 버튼 | `IgnorePaddleShift` | 패들시프트 입력 무시 |

- `SoftHoldOnly`: `AutoCruiseControl`을 켠 상태에서 소프트홀드만 사용하고, 자동 재출발·가속페달·전방차량·신호 감지에 의한 자동 크루즈 ON/OFF는 모두 사용하지 않습니다. 소프트홀드 자체가 `AutoCruiseControl != 0`을 요구하므로 오토크루즈제어는 켜 둔 채로 사용합니다. 크루즈 버튼, 패들, 블루투스·원격 명령은 영향을 받지 않습니다.
- `VisionOnly`: `EnableRadarTracks` 값과 관계없이 `-2`(VOACC)와 같은 비전 전용 경로를 사용하고, 레이더 트랙 수신 자체를 중지합니다. 자세한 내용은 [레이더](radar.md)를 참고하세요. 변경 후 재부팅이 필요합니다.
- `IgnorePaddleShift`: CAN-FD 현대·기아·제네시스에서 패들시프트 입력을 버립니다. `PaddleMode` 설정과 관계없이 패들 이벤트가 만들어지지 않습니다. 자세한 내용은 [버튼·프리셋](buttons-presets.md)을 참고하세요.
