...
class CarrotMan:
  ...
  def send_tmux_g4_web(self, tmux_why, send_settings=False):
    """Mirror the tmux log onto the owner's own upload server.

    Best effort next to the DSM and carrot_logs uploads: a failure here never
    changes whether those two counted as successful.
    """
    try:
      base_url = g4_web_upload_url()
      if not base_url:
        return None
      payload = self._tmux_upload_payload(tmux_why)
      token = create_web_upload_session_sync(base_url, payload, requests.post, "tmux")
      headers = {"Authorization": f"Bearer {token}"}
      return self._post_tmux_target("g4 tmux upload", api_url(base_url, "tmux", "upload"), headers, payload, send_settings)
    except Exception as e:
      print(f"g4 tmux sending error...: {e}")
      traceback.print_exc()
      return None
        ...
        if json_obj is None:
          ...
          if AUTO_ONROAD_DIAGNOSTICS and onroad_start_at is not None and not is_tmux_sent:
            ...
            if onroad_tmux_captured and networkConnected and now >= onroad_tmux_next_attempt_at:
              ...
              self.send_tmux_g4_web("onroad", send_settings = True)
          ...
          if pending_tmux_reason is not None and networkConnected and now >= pending_tmux_next_attempt_at:
            ...
            carrot_logs_ok = carrot_logs_response is not None and getattr(carrot_logs_response, "ok", False)
            self.send_tmux_g4_web(pending_tmux_reason, send_settings = False)
          ...
          carrot_logs_ok = carrot_logs_response is not None and getattr(carrot_logs_response, "ok", False)
          g4_response = self.send_tmux_g4_web("tmux_send") if tmux_created else None
          g4_ok = g4_response is not None and getattr(g4_response, "ok", False)
          ...
          echo = json.dumps({"tmux_send": True, "result": result, "web_ok": web_ok, "carrot_logs_ok": carrot_logs_ok,
                             "discord_ok": discord_ok, "g4_ok": g4_ok})
          socket.send(echo.encode())
      except Exception as e:
